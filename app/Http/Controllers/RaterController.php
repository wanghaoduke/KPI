...
//        \Log::info($request->all());
        $staffs = User::select([
            ...
            ->leftJoin('staff_scores', 'users.id', '=', 'staff_scores.rater_user_id')
            ->where('staff_scores.staff_user_id', $request->get('staffId'))
...
//        \Log::info($request->all());
        $raterIds = StaffScore::where('staff_user_id', $id)->where('assessment_id', $request->get('assessment_id'))->pluck('rater_user_id');
...
//        \Log::info($request->all());
        ...
        $staffScores = StaffScore::where('assessment_id',$id)
            ->where('staff_user_id',$request->get('staffId'))
            ->get();

        //添加新记录
        for($i = 0; $i < count($selectedStaffs); $i++){
            $staffScore = StaffScore::where('staff_user_id',$request->get('staffId'))
                ->where('assessment_id',$id)
                ->where('rater_user_id',$selectedStaffs[$i]['id'])
                ->get();
//            \Log::info($staffScore);
//            \Log::info($staffScore->count());
            if($staffScore->count() > 0){            //是已存在的记录 则更新
                ...
                $tempData = [];
                $tempData['percentage'] = $selectedStaffs[$i]['percentage'];
                $tempData['staff_user_id'] = $request->get('staffId');
                $tempData['rater_user_id'] = $selectedStaffs[$i]['id'];
        ...
        //已经去掉的数据 要删除
        $destoryStaffScores = StaffScore::where('staff_user_id',$request->get('staffId'))
            ->where('assessment_id',$id)
