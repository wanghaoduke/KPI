...
//        \Log::info($assessment);
        if($assessment){
            //策划组
            $planStaffs = $staffIds = StaffScore::leftJoin('users', 'users.id', '=', 'staff_scores.staff_user_id')
                ->distinct()
                ->where('staff_scores.assessment_id', $assessment->id)
                ->where("users.department", 3)
                ->pluck('staff_scores.staff_user_id');
            $showPlanStaffCount = round(count($planStaffs) / 3);   //显示员工的人数
            $planScores = AssessmentStaffFinalScore::select([
                'users.id',
                'users.name',
                'assessment_staff_final_scores.sum_score as score'
            ])->leftJoin('users', 'users.id', '=', 'assessment_staff_final_scores.user_id')
                ->where('users.department', 3)
                ->where('assessment_staff_final_scores.assessment_id', $assessment->id)
                ->orderBy('assessment_staff_final_scores.sum_score', "DESC")
                ->get();
...
//                "tt.score",
//                "tt.completed_count"
//            ])->leftJoin(\DB::raw("(select staff_scores.staff_user_id, sum(staff_scores.is_completed)as completed_count,
//                                    sum(
//                                      (
...
//                                    )as score
//                                   from staff_scores
//                                   where staff_scores.assessment_id = '$assessment->id' group by staff_scores.staff_user_id
//                                 ) as tt"),
//                'tt.staff_user_id', '=', 'users.id')
//                ->where('users.department',3)
...
//                    if($advices[$i]['suggest_user_id'] == $planScores[$j]['id']){
...
//            }
            ...
            //开发组
            $developmentStaff = $staffIds = StaffScore::leftJoin('users', 'users.id', '=', 'staff_scores.staff_user_id')
                ->distinct()
                ->where('staff_scores.assessment_id', $assessment->id)
            ...
            $developmentScores = AssessmentStaffFinalScore::select([
                'users.id',
                'users.name',
                'assessment_staff_final_scores.sum_score as score'
            ])->leftJoin('users', 'users.id', '=', 'assessment_staff_final_scores.user_id')
...
//                    if($advices[$i]['suggest_user_id'] == $developmentScores[$j]['id']){
